...
class LiveSessionConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if not await self.check_session_capacity():
            await self.close(code=1013)  # Session full
            return
    ...
    async def disconnect(self, close_code):
        ...
        # Record metrics
        if hasattr(self, 'start_time') and settings.ENABLE_METRICS:
            connection_duration = time.time() - self.start_time
            await self.record_connection_metrics(connection_duration)

        logger.info(
            f"User {self.user.id} disconnected from session {self.session_id} (code: {close_code})")
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            if settings.ENABLE_METRICS:
                ...
                await self.record_message_metrics(processing_time)
    ...
    async def check_session_capacity(self):
        """Check if session has capacity for new connections"""
        session_key = f"session_connections_{self.session_id}"
        current_connections = cache.get(session_key, 0)

        if current_connections >= settings.MAX_CONNECTIONS_PER_SESSION:
            return False

        # Increment connection count
        cache.set(session_key, current_connections + 1, 3600)
        return True

    async def decrement_connection_count(self):
        """Decrement session connection count"""
        session_key = f"session_connections_{self.session_id}"
        current_connections = cache.get(session_key, 0)
    ...
    async def check_rate_limit(self):
        ...
        key = f"rate_limit_{self.user.id}_{self.session_id}"
        current = cache.get(key, 0)

        if current >= settings.MAX_MESSAGES_PER_MINUTE:
            return False

        cache.set(key, current + 1, 60)
        return True

    async def record_message_metrics(self, processing_time):
        """Record message processing metrics"""
        try:
            cache.incr(
                f"ws_processing_time_{self.session_id}", processing_time)
            cache.incr(f"ws_message_count_{self.session_id}")

            # Alert if processing time is too high
            if processing_time > settings.SLOW_QUERY_THRESHOLD:
                logger.warning(
                    f"Slow WebSocket processing: {processing_time}s for user {self.user.id}")
        except Exception as e:
            logger.error(f"Error recording metrics: {e}")

    async def record_connection_metrics(self, duration):
        """Record connection duration metrics"""
        try:
            cache.incr(f"ws_connection_duration_{self.session_id}", duration)
            cache.incr(f"ws_connection_count_{self.session_id}")
        except Exception as e:
